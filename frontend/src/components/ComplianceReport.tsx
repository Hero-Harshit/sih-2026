...
export default function ComplianceReport({ data, onRestart }: ComplianceReportProps) {
  ...
  let riskLabel = t.report.riskLevels.low;
        ...
        <div className="grid grid-cols-1 gap-12 mt-4">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{t.report.legalGroundingTitle}</h3>
                <p className="text-slate-500 font-medium">{t.report.legalGroundingSubtitle}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {citations && citations.length > 0 ? (
                ...
                <div className="p-8 text-center bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
                  <p className="text-slate-500 font-medium">{t.report.noCitations}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{t.report.actionPlanTitle}</h3>
                <p className="text-slate-500 font-medium">{t.report.actionPlanSubtitle}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {actionPlan && actionPlan.length > 0 ? (
                actionPlan.map((action, idx) => (
                  <div key={idx} className="p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-3 animate-in slide-in-from-bottom-4 duration-700" style={{ animationDelay: `${(idx + (citations ? citations.length : 0)) * 150}ms` }}>
                ...
                <div className="col-span-full p-8 text-center bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
                  <p className="text-slate-500 font-medium">{t.report.noActions}</p>
                </div>
              )}
            </div>
          </div>
